...
def fill(
    ...
    all_polygons = []
    for sl_dict in get_soillayers(dm):
        all_polygons.append(Polygon(sl_dict["points"]))
    ...
    xmin = min([p[0] for p in fill_line])
    xmax = max([p[0] for p in fill_line])

    surfacepoints = [p for p in dm.surface if p[0] >= xmin and p[0] <= xmax]
    zmin = min([p[1] for p in surfacepoints]) - 0.1  # add small offset

    if fill_line[-1][1] > zmin:
        fill_line.append((fill_line[-1][0], zmin))
    if fill_line[0][-1] > zmin:
    ...
    for geom in geoms:
        ...
        dm.add_layer([Point(x=p[0], z=p[1]) for p in points], soil_code=fill_material)
...
for stix_file in stix_files[:10]:
    ...
    dm_new = fill(dm, "Ophoogmateriaal [17,0]", [(2, 0), (10, 0.75), (20.0, -3.5)])
    dm.serialize("test.stix")
    sys.exit(1)
