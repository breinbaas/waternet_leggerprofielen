...
class DStabilityModelModifier(BaseModel):
    ...
    soils: List[Soil] = []
    original_soil_polygons: List[SoilPolygon] = []
    ...
    def initialize(self):
        ...
        # keep a copy for the fill algorithm
        self.original_soil_polygons = [p for p in self.soil_polygons]
    ...
    def fill(self, line: List[Tuple[float, float]], soil_code: str):
        ...
        merged_polygon = unary_union(
            [spg.shapely_polygon for spg in self.original_soil_polygons]
        )

        if DEBUG:
            fig, ax = plt.subplots(figsize=(25, 10))
            ax.plot([p[0] for p in fill_line], [p[1] for p in fill_line], "k")
            ax.plot(*merged_polygon.exterior.xy, "k--")
        ...
        if DEBUG:
            fig.savefig(Path(PLOT_PATH) / f"debug.fill_line.png")
